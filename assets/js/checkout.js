$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.cart.length === 0) {
          cartContainer.html("<p style='text-align:center'>Your cart is empty.</p>");
          totalPriceContainer.text("Total: GHc 0.00");
          return;
        }

        let cartHTML = response.cart.map((item) => {
          ...
          return `
            <div class="cart-item" data-id="${item.productId}">
              <img src="https://javv.x10.mx${item.productImg}" alt="${item.productName}" class="cart-item-img">
              <div class="cart-item-details">
                <h4>${item.productName}</h4>
                <p class="cart-price">GHc ${totalItemPrice.toFixed(2)}</p>
  ...
  $(document).on("click", ".btn-remove-checkout", function () {
    ...
    $.post("https://javv.x10.mx/backend/removeFromCart.php", { product_id: productId }, function (response) {
      response = response.trim(); // ✅ trim the backend response
      if (response === "removed") {
        Toastify({
        ...
        $(".cart-count").load("https://javv.x10.mx/backend/getTotal.php", { userId: user }, function (data) {
  ...
  $("#checkOut_form").submit(function (e) {
    ...
    $.post("https://javv.x10.mx/backend/order.php", {
      username: username,
